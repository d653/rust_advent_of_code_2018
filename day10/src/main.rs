...
fn solve(v : &Vec<((i32,i32),(i32,i32))>) {
    let vf : Vec<_> = v.iter().map(|&((a,b),(c,d))| {move |i:i32|(a+i*c,b+i*d)}).collect();
    let ((x0,y0),(vx0,vy0)) = v[0];
    let ((x1,y1),(vx1,vy1)) = *v.iter().find(|(_,(vx1,vy1))| *vx1 != vx0 || *vy1 != vy0).unwrap();
...
fn main() {
    ...
    let v : Vec<_> = f.lines().map(|l|{
        ...
        ((v[1].parse().unwrap(),v[2].parse().unwrap()),(v[3].parse().unwrap(),v[4].parse().unwrap()))
    }).collect();
